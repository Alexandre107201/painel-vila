...
function renderRunrate(){
  ...
  if(dias < MIN_DIAS){
    ...
    if(Number.isFinite(f.fatAnoAnteriorTotal) && f.fatAnoAnteriorTotal > 0)
      linhas.push(`<div class="line">${f.anoAnteriorTotalLabel}: <b>${fmtR(f.fatAnoAnteriorTotal)}</b></div>`);
    elComp.innerHTML = linhas.join('');
    return;
  }
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function () {
    navigator.serviceWorker.register('./service-worker.js?v=20260915-4', { updateViaCache: 'none' }).then(function (registro) {
      registro.update();
    }).catch(function (erro) {
